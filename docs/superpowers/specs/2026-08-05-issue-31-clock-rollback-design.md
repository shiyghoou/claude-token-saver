...
1. CLI引数を従来どおり検証し、利用者指定 `out_path` を成功表示用に保持する。相対pathのfilesystem解決はengineと同じ `REPO_ROOT` 基準に揃える。
2. 解決済み最終pathの親ディレクトリが存在することを要求し、勝手に作成しない。
3. 解決済み最終pathがsymlinkなら参照先を追わず失敗する。
...
9. すべて成功した場合だけ一時ファイルを解決済み最終pathへ `mv` し、成功表示には利用者が渡した `out_path` 表記を使う。
...
- 最終 `mv` 失敗: 非ゼロで終了し、解決済み最終pathの既存出力を変更せず、private tempをcleanupする。
- 成功: 解決済み最終pathの既存出力を原子的に置換し、`書き出しました: <利用者指定パス>` を出す。
...
`test/test-token-report-launcher.sh` の実計測器fixtureへ、生成後にレポートmtimeを過去へ戻すモードを追加する。既存実装ではmarkerより新しくないため失敗し、新実装では実行専用一時ファイルの内容を根拠に成功することをRED→GREENで確認する。fixture repoのsubdirectoryからの相対 `--out` もrepo root基準で配置され、呼出元subdirectoryへ漏れないことを検証する。
