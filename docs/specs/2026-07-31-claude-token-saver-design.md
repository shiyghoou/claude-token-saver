...
現状、1 の「切り時の検知」は本リポジトリの段階3として実装済みだが、
2 の「引き継ぎ」は段階1として実装済みである。
...
- 発火源は `startup` / `clear` に限定する。設定側の `SessionStart` matcher も
...
設定は導入先の `.claude/token-saver.json` から読み、環境変数が設定ファイルより優先する。不正値は既定値へ戻す。
...
入力、設定、JSONL、状態のいずれかを読めない・書けない場合は fail-closed とし、誤った提案を出さず、
標準出力・標準エラーを空にして終了コード `0` で抜ける。外部の `jq`、Python、`timeout` には依存しない。
