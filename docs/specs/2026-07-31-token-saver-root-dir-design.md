...
`test/run.sh` に次のゲートを追加する。

- `scripts/lib/paths.sh` 以外の追跡ファイルに `.token-saver` または `.handoff`
  のリテラルが現れたら赤にする。ただし次は除外する。
  - `scripts/lib/paths.sh` 自身
  - `.gitignore`
  - `docs/`、`README.md`（説明文には現れてよい）
  - 期待値としてパスを書く必要のある `test/` 配下のファイル

除外はゲート本体に**ファイル単位で列挙する**。ワイルドカードで `test/` 全体を
免除すると、テスト側の直し忘れが検出できなくなる。

ゲートが無ければ「1箇所直し忘れ」がテスト緑のまま通る。
...
- 後片付けの `rmdir` を新パスへ変える。
  ...
  ある引き継ぎは従来どおり残る。
- **旧パスの台帳を読むフォールバックを追加する（読むだけ、書かない）。**
  これが無いと「旧版で install → 新版で uninstall」で台帳が見つからず、
  fail-closed により利用者のフックを残したまま終わる。事故ではないが、
  外せない状態になる。
