...
fence・属性エンコード・サイズ上限などの緩和があっても、引き継ぎ本文はモデル入力に載る。
**プロンプトインジェクションをコードだけで完全排除はできない**（残差リスク）。
`pending/` へ書ける主体を信頼できる作成者に限ること。出所不明なファイルは読まず、
人間が確認するか消費・削除してから新セッションを開くこと。詳細は README の
「pending の運用上の信頼境界（残差リスク）」を参照する。
