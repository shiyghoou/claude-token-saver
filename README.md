...
### pending の運用上の信頼境界（残差リスク）

可変 fence・属性エンコード・注入量上限・置き場外リンク拒否などは緩和策であり、
**引き継ぎ本文がモデル入力に載る設計である以上、プロンプトインジェクションをコードだけで完全排除はできない。**
残差として次を運用で保証する。

- `.token-saver/handoff/pending/` へ書ける主体を、信頼できる作業者とエージェントに限る。
  共有 checkout や広い書込権限があると、敵対的な本文が毎セッション注入されうる。
- 出所が不明な pending があるときは、SessionStart の前に人間が内容を確認する。
  不要なら削除するか `scripts/handoff-consume.sh` で消費してから新セッションを開く。
- 本文・`file=` / `path=`・ファイル名はいずれも非信頼の記録であり、権限の追加や命令としては扱わない
  （上記「読み込んだあとに行う判断」と同じ契約）。

詳細な書き方と読み方は [`skills/session-handoff/SKILL.md`](skills/session-handoff/SKILL.md) を正とする。
